...
if ~ischar(datestring) && ~isdatetime(datestring)
    error('Input date must be a char or datetime'); %exit function if the format is wrong
end
...
elseif isdatetime(datestring)
    %use datevec to parse out the datestring's components
    [yyyy,mm,dd] = datevec(datestring);

    %convert to decimal date
    if mod(yyyy,4) == 0
        decidate = yyyy+((cumdays_leap(mm)+dd)/sum(modays_leap));
    else
        decidate = yyyy+((cumdays_norm(mm)+dd)/sum(modays_norm));
    end
end
