...
%     if p > 21 % TOGGLE TO SELECT A SINGLE FILE FOR TESTING
        ...
        load_data = ['load ',fname,' ''m''']; eval(load_data);
        disp(['Looping through iceberg distribution #',num2str(p),' of ',num2str(nfiles)]);
        v1 = double(m.melange.Asurfs(m.melange.bergs~=0)'); dv1 = double(m.melange.binwidth(m.melange.bergs~=0)'); 
        n1 = double(m.melange.bergs(m.melange.bergs~=0)')./dv1; clear m;
        ...
        [alpha,c1,c2,c3,c4,data_lims,error] = EBC_fragmentation_curve(fname, v1, n1, dv1, norm_type, nthresh, 1, normalize_exp); % fit Eq. 1
        ...
        if use_dummy == 1
            ...
            loglog(v1, nmod_dummy, '--', 'LineWidth',1,'Color',season_cmap(str2num(fname(8:9)),:)); % plot the dummy power law
        ...
        if submarine_melt_influence == 1 
            ...
            if c1 < c5 % if the sub. melt y-intercept is greater than the original model y-intercept
                ...
                [alpha,c1,c2,c3,c4,data_lims,error] = EBC_fragmentation_curve(fname, v1_EBC, n1_EBC, dv1_EBC, norm_type, nthresh, 1, normalize_exp); % fit Eq. 1
                nmod_EBC = EBC_model([c1,c2,alpha,c3,c4],v1_EBC);

                % plot over 2nd subplot
                subplot(1,2,1); % old result
                loglog(v1,n1,'+','MarkerSize',6,'LineWidth',1,'Color',season_cmap(str2num(fname(8:9)),:)); hold on;
                loglog(v1,n_mod,'LineWidth',2,'Color',season_cmap(str2num(fname(8:9)),:)); hold on; % original model
                if use_dummy == 1
                    loglog(v1, nmod_dummy, '--', 'LineWidth',2,'Color',season_cmap(str2num(fname(8:9)),:)); %  dummy power law
                end
                xlabel('surface area [m^2]'); ylabel('count'); set(gca,'FontSize',16);
                title('Iceberg size distribution'); grid on;
                ...
                loglog(v1_EBC,nmod_EBC,'LineWidth',2,'Color',season_cmap(str2num(fname(8:9)),:)); hold on;
                xlabel('surface area [m^2]'); ylabel('count'); set(gca,'FontSize',16);
        ...
        saveas(gcf,[basepath,glacier_abbrev,'/models/',fname(1:11),'_model.png']) % save into models folder
        
        % save the parameters
        params(p,:) = [str2num(fname(4:11)), c1, c2, alpha, c3, c4, c5, c6];
